...
| `/[locale]/ai-config/agents-md` | 도구별 상세 가이드. CTA → `/ai-config/generator?tool=codex` | 동일 (CTA URL만 변경) |
| `/[locale]/ai-config/cursor` | 도구별 상세 가이드. CTA → `/ai-config/generator?tool=cursor` | 동일 (CTA URL만 변경) |
| `/[locale]/ai-config/copilot` | 도구별 상세 가이드. CTA → `/ai-config/generator?tool=copilot` | 동일 (CTA URL만 변경) |
| `/[locale]/ai-config/claude-code` | 도구별 상세 가이드. CTA → `/ai-config/generator?tool=claude-code` | 동일 (CTA URL만 변경) |
| `/[locale]/ai-config/agent-skills` | 도구별 상세 가이드. CTA → `/ai-config/generator?tool=agent-skills` | 동일 (CTA URL만 변경) |
...
`AiConfigGenerator.svelte`가 `?tool={slug}` 파라미터를 읽어 해당 도구를 Step 1에서 미리 선택된 상태로 초기화한다.

| 자식 랜딩 CTA 진입 URL | 생성기 초기 상태 |
| ---------------------- | --------------- |
| `/ai-config/generator?tool=codex` | AGENTS.md(Codex) 미리 선택됨 |
| `/ai-config/generator?tool=cursor` | Cursor 미리 선택됨 |
| `/ai-config/generator?tool=copilot` | Copilot 미리 선택됨 |
| `/ai-config/generator?tool=claude-code` | Claude Code 미리 선택됨 |
| `/ai-config/generator?tool=agent-skills` | Agent Skills 미리 선택됨 |
...
검색 유입 사용자의 전환율은 "검색 키워드 → 랜딩 콘텐츠 → CTA 진입 상태"의 일관성에 직접적으로 영향을 받는다. URL 파라미터로 도구를 사전 선택하면 Cursor 랜딩에서 유입된 사용자가 Cursor가 이미 선택된 생성기로 진입하여 추가 클릭 없이 Step 2로 진행할 수 있다.

### SEO 가치 보존

5개 자식 랜딩의 URL, 콘텐츠, JSON-LD는 변경하지 않는다. SEO 관점에서 기존 인덱싱 작업이 그대로 유효하다. 변경되는 것은 CTA URL(`/ai-config` → `/ai-config/generator?tool={slug}`)뿐이다.
...
| D'. URL 파라미터 컨텍스트 전달 | 자식 랜딩 CTA를 `?tool={slug}` 파라미터로 변경 | 맥락 단절 해결, URL 구조 유지 가능 | IA 위계 역전 근본 미해결. 옵션 C+에서 흡수 | **핵심만 흡수 (C+에 통합)** |
...
- `AiConfigGenerator.svelte`에 `?tool` URL 파라미터 읽기 + Step 1 초기화 로직 추가
- 5개 자식 랜딩의 CTA URL을 `/ai-config/generator?tool={slug}`로 변경
...
- §3.2.1: URL 파라미터 초기화 로직 명시
- §3.3 사용자 플로우: 경로 A/B, URL 파라미터 매핑 추가
...
Phase B에서 Shareable URL을 구현할 때, `/ai-config/generator?tool=cursor`와 Shareable URL 파라미터의 충돌 또는 통합을 설계해야 한다. 예를 들어 `/ai-config/generator?tool=cursor&state=...` 형태 또는 파라미터 우선순위 규칙이 필요하다. 이 설계는 Phase B 착수 시 별도 검토한다.
...
- **`?tool` 파라미터 유효성 검증**: 정의되지 않은 slug 값으로 진입 시 기본 상태로 폴백하는 로직이 필요하다
