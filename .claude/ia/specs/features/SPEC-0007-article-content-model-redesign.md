...
| 4. 스키마/UX 보강 | `feature/1.6.0-article-schema` | BlogPosting JSON-LD 확장 + UX 변경(상단 라벨/하단 CTA) + article 메타 태그 | 페이즈 3 머지 |
| 5. 기존 124개 삭제 + sample 발행 검증 | `feature/1.6.0-article-cleanup` | `git rm` + `.gitkeep` + sample 10건 수동 실행/검토 | 페이즈 3·4 머지 |
...
- [ ] 페이즈 1 머지 전: SPEC + ADR 내용 사용자 확정
- [ ] 페이즈 2 머지 전: 원문 fetch 성공률 실측 (15개 소스 sample → 80%+ 목표)
- [ ] 페이즈 3 머지 전: sample 5건 generation 결과 사용자 검토
- [ ] 페이즈 5 머지 전: 기존 124개 삭제 직전 사용자 최종 확인
- [ ] v1.6.0 릴리즈 후: 신규 발행이 충분히 누적될 때까지 `noindex` 유지
