...
related_adrs:
  ...
  - ADR-0017  # AI 도구 설정 파일 형식 우선순위 (AGENTS.md 1순위)
  - ADR-0018  # Agent Skills 카탈로그 채택 및 P0 8종 범위
  - ADR-0019  # AI Config IA 재설계: 카탈로그 + 통합 생성기 분리, URL 파라미터 컨텍스트 전달
related_specs:
  - SPEC-0003  # Shareable URL — Phase B에서 연동 (카탈로그→생성기 ?focus 파라미터와 연관)
...
- **[IA 재설계 — ADR-0019]** `/ai-config` 카탈로그 허브 페이지 신설 (5개 도구 카드 + 통합 생성기 진입 CTA)
- **[IA 재설계 — ADR-0019]** `/ai-config/generator` 통합 생성기 페이지 (기존 `AiConfigGenerator.svelte` 이전)
- **[IA 재설계 — ADR-0019]** `AiConfigGenerator.svelte`에 `?focus={slug}` URL 파라미터 비대칭 초기화 로직 추가
- **[IA 재설계 — ADR-0019]** 5개 자식 랜딩 CTA URL을 `/ai-config/generator?focus={slug}` 형식으로 변경
- **[IA 재설계 — ADR-0019]** 카탈로그 페이지용 한/영 i18n 카피 추가
...
**[2026-05-01 IA 재설계 반영 — ADR-0019]**

초기 SPEC에서는 `/ai-config`를 통합 생성기 허브로 직접 사용하는 방식을 택했다. 그러나 5개 SEO 랜딩 페이지(`/ai-config/cursor` 등) 구현 완료 후, 부모-자식 위계 역전 문제가 발견되었다:

- **문제**: 부모(`/ai-config`)가 복잡한 인터랙티브 생성기이고 자식(`/ai-config/cursor`)이 단순 SEO 랜딩이어서 IA 위계가 역전됨. 일반적 IA 패턴에서 부모는 카탈로그/개요, 자식은 상세여야 한다
- **추가 문제**: 자식 랜딩의 CTA("생성기 시작하기 →")가 `/ai-config`로 연결되지만 도구 사전 선택 없이 진입하므로, 예를 들어 Cursor 랜딩에서 진입해도 Cursor가 미리 선택된 상태가 아니어서 검색 → 랜딩 → 생성기 진입 경로의 맥락이 단절됨

이에 따라 **옵션 C+(ADR-0019)**를 채택하여 구조를 재설계한다:

- `/ai-config` → **카탈로그 허브** (5개 도구 카드 + 통합 생성기 진입 CTA)
- `/ai-config/generator` → **통합 생성기** (현재 `AiConfigGenerator.svelte` 이전)
- 자식 랜딩 CTA → `/ai-config/generator?focus={slug}` (랜딩 컨텍스트에 맞는 초기 상태로 진입)
...
> **[ADR-0019]** 통합 생성기는 `/ai-config/generator`에서 동작한다. 자식 랜딩(`/ai-config/cursor` 등)에서 CTA를 통해 `?focus={slug}` 파라미터로 진입 시, `AiConfigGenerator.svelte`가 해당 파라미터를 읽어 랜딩 컨텍스트에 맞는 초기 상태로 설정한다. 파라미터 이름이 `?tool`이 아닌 `?focus`인 이유는 이 파라미터가 단순한 도구 사전 선택이 아니라 "이 랜딩이 강조하는 컨텍스트"를 전달하기 때문이다. 각 `?focus` 값에 따른 초기 상태는 §3.3의 매핑 표를 따른다.
...
**Phase A 기본 플로우 [ADR-0019 반영]**:

**경로 A — 카탈로그 직접 진입**:

1. `/ai-config` 카탈로그 허브 진입 (또는 `/ko/ai-config`)
2. "통합 생성기 시작하기" CTA 클릭 → `/ai-config/generator` 진입
3. 생성기에서 도구 선택(Step 1) 후 진행

**경로 B — 도구별 랜딩에서 진입 (검색 유입)**:

1. 검색에서 `/ai-config/cursor` 랜딩 도달
2. CTA "Cursor Rules 생성기 시작하기 →" 클릭 → `/ai-config/generator?focus=cursor` 진입
3. 생성기 Step 1에서 Cursor만 선택된 상태로 초기화됨 + Cursor MDC 미리보기 탭 활성화 → 즉시 Step 2로 진행 가능
4. 다른 도구도 추가로 선택 가능 (Cursor만 단독 사용 제한 없음)

**`?focus` 파라미터 슬러그 매핑 (ADR-0019 §URL 파라미터 컨텍스트 전달 참조)**:

| 자식 랜딩 | CTA URL | 생성기 초기 상태 |
| --------- | ------- | --------------- |
| `/ai-config/agents-md` | `/ai-config/generator?focus=agents-md` | 4개 도구 모두 선택(Cursor, Copilot, Claude Code, Codex) + AGENTS.md 미리보기 탭 활성화 |
| `/ai-config/cursor` | `/ai-config/generator?focus=cursor` | Cursor만 선택 + Cursor MDC 미리보기 탭 활성화 |
| `/ai-config/copilot` | `/ai-config/generator?focus=copilot` | Copilot만 선택 + Copilot 미리보기 탭 활성화 |
| `/ai-config/claude-code` | `/ai-config/generator?focus=claude-code` | Claude Code 선택(AGENTS.md 임포트 옵션 활성화, CP-3 기준) + CLAUDE.md 미리보기 탭 활성화 |
| `/ai-config/agent-skills` | `/ai-config/generator?focus=agent-skills` | 도구 선택은 사용자 기본값 유지 + Step 2 Agent Skills 영역이 펼쳐진 상태 |

**생성기 내 기본 플로우**:

1. **1단계**: 스택 선택 — 프레임워크, 언어, 패키지 매니저, 테스트 도구 (URL 파라미터로 진입 시 해당 도구 사전 선택됨)
2. **2단계**: 베스트 프랙티스 체크리스트 — Code Style / Git Workflow / Testing 섹션별 선택
3. **3단계** (선택): Boundaries 3-tier 커스터마이징
4. 미리보기 탭: AGENTS.md / Cursor MDC / Copilot / CLAUDE.md / Skills 탭 전환으로 각 산출물 확인
5. 다운로드:
...
- Phase B Shareable URL(SPEC-0003)은 `/ai-config/generator?focus=cursor&...` 형태로 URL 파라미터와 통합 설계 필요 (ADR-0019 §결과 참조)
...
| 대안 | 설명 | 장점 | 단점 | 채택 여부 |
|------|------|------|------|-----------|
| A. 도구별 분리 페이지 | `/ai-config/cursor`, `/ai-config/copilot` 등 도구별 개별 생성기 | 도구별 SEO 최적화 용이, 구현 단순 | 사용자 4번 방문 필요, DRY 위반, 통합 포지셔닝 불가 | **조건부 채택** → 아래 "대안 A 결정 보완" 참조 |
...
**대안 A 결정 보완 [2026-05-01 IA 재검토 반영]**

초기 SPEC에서는 대안 A를 "불채택"으로 기록하면서도 "상세 페이지는 SEO용으로 유지, 생성기 자체는 통합"이라는 절충안을 택했다. 이 결정의 구체적 형태는 `/ai-config` = 통합 생성기, `/ai-config/cursor` 등 = SEO 랜딩이었다.

그러나 5개 SEO 랜딩 페이지 구현 완료 후 IA를 검토한 결과, 이 구조에서 두 가지 문제가 발견되었다:

1. **부모-자식 위계 역전**: 부모(`/ai-config`)가 복잡한 인터랙티브 생성기이고 자식이 단순 정적 랜딩. 일반적인 IA에서 부모는 카탈로그/개요, 자식은 상세여야 한다. 이 역전 구조는 헤더 네비게이션("AI Config Files" → `/ai-config`)이 생성기 직접 진입으로 해석되어 혼란을 유발한다.
2. **맥락 단절**: 자식 랜딩의 CTA가 `/ai-config`로 연결되지만 도구 사전 선택이 없어 "Cursor 랜딩 → 생성기 진입 → Cursor가 선택되지 않은 상태"라는 검색 유입 경험 단절이 발생한다.

이에 따라 **옵션 C+(ADR-0019)를 새로운 채택 방향으로 결정**한다:

- 도구별 페이지(대안 A의 상세 랜딩)는 **그대로 유지** (SEO 가치 보존)
- `/ai-config`를 **카탈로그 허브로 전환** (5개 도구 카드 + 통합 생성기 CTA)
- 통합 생성기를 **`/ai-config/generator`로 분리** (IA 위계 정상화)
- 자식 랜딩 CTA를 **`/ai-config/generator?focus={slug}`로 변경** (맥락 단절 해결)

SEO 처리: 현 시점 미배포 상태(2026-05-01 도입, 상용 배포 전)이므로 301 redirect 등 별도 SEO 처리 불필요. 사용자(jsg3121) 명시적 결정.
...
| 단계 | 작업 | 산출물 | 선행 조건 | 상태 |
| ---- | ---- | ------ | --------- | ---- |
| 1 | SPEC-0005 검토 및 승인 | 승인된 SPEC | - | 완료 |
| 2 | 후속 ADR 2개 작성 (채택 형식 우선순위, Skills 카탈로그) | ADR-0017, ADR-0018 | 1 | 완료 |
...
| 11 | SEO 랜딩 페이지 신설 (`/ai-config/cursor` 등 5종) | Astro 페이지, JSON-LD, hreflang | 8 | 완료 |
| 12 | **[ADR-0019]** IA 재설계: 카탈로그 + 생성기 분리, URL 파라미터 컨텍스트 전달 | `/ai-config/generator` 신규 페이지 생성·`AiConfigGenerator.svelte` 이전, `/ai-config` 카탈로그 재구성, `?focus` 파라미터 비대칭 초기화 로직, 5개 랜딩 CTA URL 변경, 카탈로그 i18n 카피 | 11 | 착수 예정 |
| 13 | 단위 테스트 (생성 로직) | Vitest 테스트 | 4~7 | - |
| 14 | E2E 테스트 (생성 + 다운로드 플로우) | Playwright 테스트 | 8, 9 | - |
...
> **[ADR-0019 반영]** `/ai-config`는 카탈로그 허브, `/ai-config/generator`가 통합 생성기. 자식 랜딩 → 생성기 진입 시 `?focus={slug}` 파라미터로 랜딩 컨텍스트 전달. 파라미터 이름이 `?tool`이 아닌 `?focus`인 이유와 비대칭 초기화 상태는 ADR-0019 §URL 파라미터 컨텍스트 전달 참조.

| 페이지 | 한국어 URL | 영어 URL | 역할 |
| ------ | ---------- | -------- | ---- |
| 카탈로그 허브 | `/ko/ai-config` | `/en/ai-config` | 5개 도구 카드 + 통합 생성기 CTA |
| 통합 생성기 | `/ko/ai-config/generator` | `/en/ai-config/generator` | 실제 생성기 (신규) |
| AGENTS.md 랜딩 | `/ko/ai-config/agents-md` | `/en/ai-config/agents-md` | SEO 상세 가이드. CTA → `?focus=agents-md` |
| Cursor 설정 랜딩 | `/ko/ai-config/cursor` | `/en/ai-config/cursor` | SEO 상세 가이드. CTA → `?focus=cursor` |
| Copilot 인스트럭션 랜딩 | `/ko/ai-config/copilot` | `/en/ai-config/copilot` | SEO 상세 가이드. CTA → `?focus=copilot` |
| Claude Code CLAUDE.md 랜딩 | `/ko/ai-config/claude-code` | `/en/ai-config/claude-code` | SEO 상세 가이드. CTA → `?focus=claude-code` |
| Agent Skills 랜딩 | `/ko/ai-config/agent-skills` | `/en/ai-config/agent-skills` | SEO 상세 가이드. CTA → `?focus=agent-skills` |
...
> **[ADR-0019 반영]** 페이지 역할이 분리되었으므로 JSON-LD 스키마도 역할에 맞게 분리한다.

| 페이지 | JSON-LD 스키마 | 설명 |
| ------ | -------------- | ---- |
| `/ai-config` 카탈로그 허브 | `CollectionPage` + `ItemList` (5개 도구) | 카탈로그 성격에 맞는 컬렉션 스키마 |
| `/ai-config/generator` 통합 생성기 | `SoftwareApplication` | 실제 기능 애플리케이션 스키마 |
| 5개 자식 랜딩 | `SoftwareApplication` | 기존 생성기 랜딩 패턴 동일 적용 |
...
- [ADR-0019 AI Config IA 재설계](../../decisions/records/ADR-0019-ai-config-ia-redesign.md) — 카탈로그 + 통합 생성기 분리 결정 근거
...
| 2026-05-01 | IA 위계 역전 발견 및 해소. 옵션 C+(ADR-0019) 채택 반영. 갱신 섹션: frontmatter(status·related_adrs), §2.1 In Scope(IA 재설계 항목 추가), §3.1 개요(카탈로그+생성기 분리 구조 명시), §3.2.1(URL 파라미터 초기화 로직), §3.3 사용자 플로우(경로 A/B·URL 파라미터 매핑), §5 대안 A 결정 보완, §6.1 단계(단계 11 완료·신규 단계 추가), §9.1 URL 구조, §9.4 JSON-LD, §11 참고 자료 | product-planner |
| 2026-05-01 | URL 파라미터 `?tool` → `?focus` 전면 보정. `agents-md`/`agent-skills`의 의미적 모순 해소. 자식 랜딩별 비대칭 초기화 매핑 반영(ADR-0019 §URL 파라미터 컨텍스트 전달 참조). 갱신 섹션: frontmatter(related_specs), §2.1(파라미터명 수정), §3.1(CTA URL 표기), §3.2.1(파라미터 설명), §3.3(슬러그 매핑 표 5행 + 초기 상태 컬럼 추가), §5 대안 A, §6.1 단계 12, §9.1 URL 구조 | product-planner |
