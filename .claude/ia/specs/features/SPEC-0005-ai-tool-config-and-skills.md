...
- **M1**: 후속 ADR 2개 승인 + 데이터 모델/타입 정의 완료
...
> 아래 항목은 구현 착수 전 사용자 확인이 필요하다. 각 지점에서 반드시 멈추고 승인을 받는다.

- [ ] **CP-1**: 2단계 입력 방식 — 카탈로그 체크리스트 단독 vs 자유 텍스트 병행 (권장: 카탈로그 단독)
- [ ] **CP-2**: Cursor MDC Phase A 파일 분리 기준 — 3파일 고정 자동(권장) vs 사용자 토픽 지정
- [ ] **CP-3**: CLAUDE.md `@AGENTS.md` 임포트 — 자동 삽입(권장) vs 사용자 선택
- [ ] **CP-4**: Skills frontmatter — 공통 필드만(권장) vs Claude 확장 필드 Phase A 노출
- [ ] **CP-5**: 출력 패키징 — ZIP + 개별 복사 병행(권장) vs ZIP 단독
- [ ] **CP-6**: Skills와 설정 파일 — 한 페이지 동시 생성(권장) vs 분리 페이지
- [ ] **CP-7**: ZIP 라이브러리 선택 — ADR 작성 필요 여부 판단 (JSZip 사용 시 ADR 필요)
