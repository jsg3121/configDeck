...
related_specs:
  - SPEC-0003  # Shareable URL — Phase B에서 연동 (카탈로그→생성기 ?tool 파라미터와 연관)
...
- **[IA 재설계 — ADR-0019]** `AiConfigGenerator.svelte`에 `?tool={slug}` URL 파라미터 초기화 로직 추가
- **[IA 재설계 — ADR-0019]** 5개 자식 랜딩 CTA URL을 `/ai-config/generator?tool={slug}` 형식으로 변경
...
- 자식 랜딩 CTA → `/ai-config/generator?tool={slug}` (도구 사전 선택 상태로 진입)
...
> **[ADR-0019]** 통합 생성기는 `/ai-config/generator`에서 동작한다. 자식 랜딩(`/ai-config/cursor` 등)에서 CTA를 통해 `?tool={slug}` 파라미터로 진입 시, `AiConfigGenerator.svelte`가 해당 파라미터를 읽어 Step 1의 도구 선택이 해당 도구로 미리 선택된 상태로 초기화된다. 예를 들어 `/ai-config/generator?tool=cursor`로 진입하면 Step 1에서 Cursor가 이미 선택된 상태로 시작된다.
...
2. CTA "Cursor Rules 생성기 시작하기 →" 클릭 → `/ai-config/generator?tool=cursor` 진입
3. 생성기 Step 1에서 Cursor가 미리 선택된 상태로 초기화됨 → 즉시 Step 2로 진행 가능
...
| 자식 랜딩 | CTA URL |
| --------- | ------- |
| `/ai-config/agents-md` | `/ai-config/generator?tool=codex` |
| `/ai-config/cursor` | `/ai-config/generator?tool=cursor` |
| `/ai-config/copilot` | `/ai-config/generator?tool=copilot` |
| `/ai-config/claude-code` | `/ai-config/generator?tool=claude-code` |
| `/ai-config/agent-skills` | `/ai-config/generator?tool=agent-skills` |
...
- Phase B Shareable URL(SPEC-0003)은 `/ai-config/generator?tool=cursor&...` 형태로 URL 파라미터와 통합 설계 필요 (ADR-0019 §결과 참조)
...
- 자식 랜딩 CTA를 **`/ai-config/generator?tool={slug}`로 변경** (맥락 단절 해결)
...
| 12 | **[ADR-0019]** IA 재설계: 카탈로그 + 생성기 분리, URL 파라미터 컨텍스트 전달 | `/ai-config/generator` 신규 페이지 생성·`AiConfigGenerator.svelte` 이전, `/ai-config` 카탈로그 재구성, `?tool` 파라미터 초기화 로직, 5개 랜딩 CTA URL 변경, 카탈로그 i18n 카피 | 11 | 착수 예정 |
...
> **[ADR-0019 반영]** `/ai-config`는 카탈로그 허브, `/ai-config/generator`가 통합 생성기. 자식 랜딩 → 생성기 진입 시 `?tool={slug}` 파라미터로 도구 컨텍스트 전달.
...
| AGENTS.md 랜딩 | `/ko/ai-config/agents-md` | `/en/ai-config/agents-md` | SEO 상세 가이드. CTA → `?tool=codex` |
| Cursor 설정 랜딩 | `/ko/ai-config/cursor` | `/en/ai-config/cursor` | SEO 상세 가이드. CTA → `?tool=cursor` |
| Copilot 인스트럭션 랜딩 | `/ko/ai-config/copilot` | `/en/ai-config/copilot` | SEO 상세 가이드. CTA → `?tool=copilot` |
| Claude Code CLAUDE.md 랜딩 | `/ko/ai-config/claude-code` | `/en/ai-config/claude-code` | SEO 상세 가이드. CTA → `?tool=claude-code` |
| Agent Skills 랜딩 | `/ko/ai-config/agent-skills` | `/en/ai-config/agent-skills` | SEO 상세 가이드. CTA → `?tool=agent-skills` |
