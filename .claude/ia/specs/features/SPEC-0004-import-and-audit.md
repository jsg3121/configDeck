...
```typescript
// src/lib/migration/types.ts (예정)
export interface ConfigInspector<TLegacy, TModern> {
...
| 6 | **B** | **Audit-only 진입 동선 추가** | `/migration` 허브 페이지, MigrationPanel 모드 분기 | 5 |
...
- [ ] M2 시작 전: 공통 인터페이스(`ConfigInspector`) 설계 검토 → 사용자 승인
